...
results_within <- run_analysis(snvs_prefiltered, anchor = anchor) %>%
    filter(same_individual == "Yes") %>%                 # keep only within-individual comparisons
...
donor_data <- snvs_prefiltered %>%
    ...
    select(scaffold, position,
           ...
           total_pos_donor = total_positions) %>%
    distinct()
...
results_donor <- snvs_prefiltered %>%
    ...
    mutate(
        ...
        timepoint_pair = paste0(anchor, "_vs_", timepoint),
        min_total      = pmin(total_positions, total_pos_donor)
    ) %>%
...
combined_proportions <- bind_rows(
        results_within %>% select(scaffold, position, same_variant,
...
ks_results <- combined_proportions %>%
    ...
    map_dfr(function(df) {
        ...
        within <- df %>% filter(group == "Within individual") %>% pull(prop_combined)
        donor  <- df %>% filter(group == "Donor")             %>% pull(prop_combined)

        test <- ks.test(within, donor)
...
p <- combined_proportions %>%
    ...
    labs(
        ...
        fill    = NULL,
        title   = "SNV sharing proportion — within individuals vs donor",
...
ggsave(filename = "Shared_SNVs_position_freq_within_donor.pdf", plot = p, height = 8, width = 6)