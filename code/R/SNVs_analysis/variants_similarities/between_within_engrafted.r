...
# ---- Run within and between from anchor ----
results_0 <- run_analysis(snvs_prefiltered, anchor = anchor)

results_within <- results_0 %>%
...
stopifnot(
    ...
    all(results_between %>% distinct(timepoint_pair) %>% pull() %in% tp_order),
...
all_results <- bind_rows(
    ...
    mutate(
        ...
        group          = factor(group, levels = c("Within individual",
                                                   "Between individuals",
                                                   "Donor"))
    )

# ---- Contingency tables per timepoint pair ----
contingency_tables <- all_results %>%
    group_by(timepoint_pair, group, same_variant) %>%
    count() %>%                                          # Count positions in each group×same_variant combination
    ungroup() %>%
    pivot_wider(names_from = same_variant,               # Reshape to wide: same_variant becomes columns
                values_from = n,
                values_fill = 0) %>%
    mutate(
        total     = Yes + No,                            # Total positions per group per timepoint pair
        prop_same = Yes / total                          # Proportion sharing same variant base
    )

contingency_tables %>% view()

# ---- Statistical test per timepoint pair (overall across all three groups) ----
fisher_all <- all_results %>%
    group_by(timepoint_pair, group, same_variant) %>%
    count() %>%                                          # Count positions in each group×same_variant combination
    ungroup() %>%
    ...
    map_dfr(function(df) {
        ...
        test <- if (any(mat < 5))                        # Adaptive test: Fisher's exact with Monte Carlo
                    fisher.test(mat,                     # simulation for small counts (handles 3-column tables),
                                simulate.p.value = TRUE) # chi-square for large counts
                else chisq.test(mat)
    ...
    mutate(
        ...
        sig_label = case_when(                           # Convert adjusted p-value to significance label
            p_adj < 0.001 ~ "***",
            p_adj < 0.01  ~ "**",
            p_adj < 0.05  ~ "*",
            TRUE          ~ "ns"
...
dumbbell_all <- all_results %>%
    ...
    left_join(                                           # Add significance labels
        fisher_all %>% select(timepoint_pair, sig_label),
        by = "timepoint_pair") %>%
...
max_prop <- max(dumbbell_all$prop, na.rm = TRUE)         # For positioning significance labels

# ---- Dumbbell plot ----
p <- dumbbell_all %>%                                    # Assign to object for safe ggsave
    ggplot(aes(x = prop, y = timepoint_pair)) +
    geom_line(aes(group = timepoint_pair),               # Line connecting three group points per timepoint pair
    ...
    labs(
        ...
        colour  = NULL,
        title   = "Variant base sharing — within, between and donor",
        caption = "Significance indicates overall difference across all three groups\n* p.adj < 0.05, ** p.adj < 0.01, *** p.adj < 0.001"
    ) +
    theme_bw() +
    theme(
        legend.position  = "top",
...
# ---- Save plot ----
ggsave(
    filename = "~/PIMMSgit/plots/Within_between_donor_variant_sharing.pdf",
    plot     = p,
    width    = 8,
    height   = 6
)